...
query <- readRDS("data/querys/unnorm_sc_C32_notreatment.rds")
names(assays(query))=c("counts")


query1 <- readRDS("data/querys/unnorm_sc_C32-24h.rds")
names(assays(query1))=c("counts")

query2 <- readRDS("data/querys/unnorm_sc_C32-72h.rds")
...
for (cell_type in cell_types) {
  ...
  genes <- predictions@metadata$de.genes$FCDI_Astro[[cell_type]]
...
genes_with_8_occurrences <- names(gene_counts)[sapply(gene_counts, function(x) x == 2)]

genes_with_8_occurrences
